 ...
 *
 * Based on Phaze phase vocoder (Unlicense) by olvb
 * https://github.com/olvb/phaze
 *
 * Inlines fft.js (MIT) for self-contained AudioWorklet usage.
 */

// ============================================================
// Inline fft.js (https://github.com/nicklasoverworlds/fft.js)
// ============================================================
...
// ============================================================
// OLA (Overlap-Add) Base Processor
// Based on Phaze by olvb (Unlicense)
// ============================================================
...
class OLAProcessor extends AudioWorkletProcessor {
  ...
  allocateInputChannels(inputIndex, nbChannels) {
    ...
    for (var i = 0; i < nbChannels; i++) {
      this.inputBuffersHead[inputIndex][i] = new Float32Array(this.blockSize);
      this.inputBuffersHead[inputIndex][i].fill(0);
    }

    this.inputBuffersToSend[inputIndex] = new Array(nbChannels);
    for (var i = 0; i < nbChannels; i++) {
      this.inputBuffersToSend[inputIndex][i] = new Float32Array(this.blockSize);
      this.inputBuffersToSend[inputIndex][i].fill(0);
    }
  }
  ...
  reallocateChannelsIfNeeded(inputs, outputs) {
    for (var i = 0; i < this.nbInputs; i++) {
      ...
      if (this.inputBuffers[i].length !== nbChannels) {
    ...
    for (var i = 0; i < this.nbOutputs; i++) {
      ...
      if (nbChannels === 0) nbChannels = 1;
      if (this.outputBuffers[i].length !== nbChannels) {
  ...
  prepareInputBuffersToSend() {
    for (var i = 0; i < this.nbInputs; i++) {
      for (var j = 0; j < this.inputBuffers[i].length; j++) {
        this.inputBuffersToSend[i][j].set(this.inputBuffers[i][j].subarray(0, this.blockSize));
      }
    }
  }
  ...
  processOLA(_inputs, _outputs, _params) {
...
// ============================================================
// Phase Vocoder Pitch Shift Processor
// Based on Phaze by olvb (Unlicense)
// ============================================================
...
class PitchShiftProcessor extends OLAProcessor {
  ...
  processOLA(inputs, outputs, parameters) {
    var pitchFactor = parameters.pitchFactor[parameters.pitchFactor.length - 1];
  ...
  findPeaks() {
    this.nbPeaks = 0;
    var i = 2;
    var end = this.magnitudes.length - 2;
  ...
  shiftPeaks(pitchFactor) {
    ...
    for (var i = 0; i < this.nbPeaks; i++) {
      ...
      if (i > 0) {
        var peakIndexBefore = this.peakIndexes[i - 1];
        startIndex = peakIndex - Math.floor((peakIndex - peakIndexBefore) / 2);
      }
      if (i < this.nbPeaks - 1) {
        var peakIndexAfter = this.peakIndexes[i + 1];
        endIndex = peakIndex + Math.ceil((peakIndexAfter - peakIndex) / 2);
      }

      var startOffset = startIndex - peakIndex;
      var endOffset = endIndex - peakIndex;
      for (var j = startOffset; j < endOffset; j++) {
        var binIndex = peakIndex + j;
        var binIndexShifted = peakIndexShifted + j;

        if (binIndexShifted >= this.magnitudes.length) break;
        if (binIndex < 0 || binIndex >= this.magnitudes.length) continue;
        ...
        var indexReal = binIndex * 2;
        var indexImag = indexReal + 1;
        ...
        var indexShiftedReal = binIndexShifted * 2;
        var indexShiftedImag = indexShiftedReal + 1;
